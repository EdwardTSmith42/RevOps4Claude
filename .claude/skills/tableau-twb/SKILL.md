...
76. **A dashboard `type-v2='filter'` zone renders NOTHING unless it carries `name='<owning worksheet name>'`.** This is the single highest-impact filter-card bug and it is invisible to static validation: the workbook publishes clean, Cloud renders it, lint passes, and the user simply has no filter controls. Proven by A/B on one dashboard (two byte-identical zones differing only in `name=`; the one with it drew a card, the one without drew a blank gap) and by corpus (287 of 287 filter zones across Desktop-authored files carry `name=`, zero lack it). Every other prerequisite can already be correct -- worksheet `<filter>` elements, worksheet window `<card type='filter'>`, dashboard `<datasource-dependencies>` with matching column + column-instance, correct `:nk`/`:qk` suffixes -- and the card still will not paint. **Emit `name=` on every filter zone and assert it at build time against the worksheet zones on that same dashboard.**

77. **An unrecognized `mode` on a filter zone does NOT error -- it silently falls back to a scrolling checklist.** That is how a generated dashboard ends up as "a big list instead of a drop down". Only these are proven to render as intended: `checkdropdown` (multi-select compact dropdown -- 222 of 228 Desktop instances), `dropdown` (single-select compact), `radiolist` (radio buttons), `checklist` (the scrolling list). `compact` and `typeinandsearch` both published cleanly and fell back to checklist. `pattern` and `slider` rendered compact but appear in zero Desktop files -- do not ship them. **Whitelist mode and raise on anything else.** Choose by cardinality: >5 values -> `checkdropdown`, 2-5 -> `radiolist`/`checklist`.

78. **A numeric range slider needs no `mode` at all.** Use a `:qk` column-instance (`derivation='None' type='quantitative'`), a worksheet `<filter class='quantitative'>` with `included-values='in-range'` (or `'all'`, which renders as "All values"), the `:qk` column inside `<slices>`, and a zone with NO `mode` plus `show-domain='true' show-null-ctrls='true'`.

79. **`custom-title='true'` is silently ignored on a filter zone; it only works on `paramctrl` zones.** A filter card's title is the field caption. To change it without republishing the datasource, add a workbook-local `<column caption='...'>` override on the workbook's `<datasource>` element (after `<aliases>`, before `<layout>`). Overriding the caption inside `<datasource-dependencies>` does NOT work.

80. **A `layout-flow` container distributes children evenly and ignores per-zone `h` and `fixed-size`.** A filter rail built as `layout-flow param='vert'` renders every card at a uniform pitch regardless of the heights you asked for. Build filter rails as an absolute `layout-basic` with explicit `x/y/w/h`. Desktop uses `fixed-size` with `is-fixed` only on the flow axis of a `layout-flow`.

81. **Two `[:Measure Names]` aliases with the SAME text silently drop one, killing a column header.** Aliasing two different measures to the same display string (e.g. two fields both aliased "Max loan") makes one header vanish with no error. Keep alias values unique per datasource and assert it at build time. Related: a single measure placed on Text draws no column header at all -- move it to `[:Measure Names]` to get one.

82. **`<slices>` membership is what makes a filter apply to every sheet on a datasource.** A filter that exists on a worksheet but is not in that worksheet's `<slices>` block is sheet-scoped: the card moves one sheet and leaves every KPI tile on the same dashboard untouched. For the house convention (one card per filter, applied to all worksheets using that data source), emit the filter column into the `<slices>` block of EVERY worksheet on that datasource.

83. **Selecting one item out of a huge dimension: use a string parameter plus a CONTAINS calc, not a filter card.** A filter card binds to one datasource; a parameter is workbook-global, so one search box can scope sheets built on three different datasources. Pattern: string parameter (`param-domain-type='any'`, empty default) + a boolean calc `LEN(TRIM([Parameter 1])) = 0 OR CONTAINS(UPPER([NameField]), UPPER(TRIM([Parameter 1]))) OR ...` filtered to true and listed in `<slices>`, with a `paramctrl` zone `mode='type_in'`. The empty default short-circuits to true so nothing is hidden and no tab opens blank. Then a downstream single-select card with `values='relevant'` holds only the matches -- Only Relevant Values becomes usable precisely because the parameter pre-narrowed the domain. Cap `values='relevant'` at about 2 cards per dashboard; each one issues its own domain query on every interaction.
